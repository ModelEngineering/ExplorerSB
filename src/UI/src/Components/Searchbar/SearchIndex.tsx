...
const SearchIndex = ({
  ...
  useEffect(() => {
    ...
    setResult(results.map((result) => {
      ...
      contextValue = contextValue.slice(0, index) + "**" + contextValue.slice(index);
      length += 2
      contextValue = contextValue.slice(0, index + length) + "**" + contextValue.slice(index + length);
      length += 2
      const text = contextValue.substring(index - 40, index + length + 40)
      const startIndex = index - 40
      const endIndex = index + length + 40
      let textTrimmed = text.slice(text.indexOf(" "), text.lastIndexOf(" ")).trim()
      if(startIndex > 0){
